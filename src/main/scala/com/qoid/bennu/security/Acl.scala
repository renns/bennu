...
class Acl(labelAcl: LabelAcl, injector: ScalaInjector) {
  ...
  private def loadAliasIids(): Unit = {
    if (!aliasesLoaded) {
      AgentSecurityContext(labelAcl.agentId) {
        _reachableAliasIids = injector.instance[JdbcConn].queryFor[InternalId](sql"""
          select iid
          from alias
          where labelIid in (${_reachableLabelIids})
        """).toList
      }
  ...
  private def loadConnectionIids(): Unit = {
    if (!connectionsLoaded) {
      AgentSecurityContext(labelAcl.agentId) {
        _reachableConnectionIids = injector.instance[JdbcConn].queryFor[InternalId](sql"""
          select iid
          from connection
          where aliasIid in (${_reachableAliasIids})
        """).toList
      }
  ...
  private def loadContentIids(): Unit = {
    if (!contentLoaded) {
      AgentSecurityContext(labelAcl.agentId) {
        _reachableContentIids = injector.instance[JdbcConn].queryFor[InternalId](sql"""
          select contentIid
          from labeledcontent
          where labelIid in (${_reachableLabelIids})
        """).toList

...
trait AgentView {
  ...
  def findChildLabel(parentIid: InternalId, childName: String): Box[Label] = {
    val iids = LabelChild.select(sql"parentIid = ${parentIid} and deleted = false")(inject[JdbcConn]).map(_.childIid).toList
    selectBox[Label](sql"name = ${childName} and iid in (${iids})")
  }
